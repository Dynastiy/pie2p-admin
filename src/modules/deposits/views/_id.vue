<template>
  <div class="transaction-details spacer">
    ...
    <div v-else>
      <div class="tw-bg-white tw-p-4">
        ...
        <h4 class="tw-font-semibold tw-mt-3">
          Deposit
        ...
        <div>
          ...
          <div v-if="item.transaction">
            <div>
              ...
              <p class="details-body">
                {{
                  item.transaction.currency.symbol +
                  Number(item.transaction.amount).toLocaleString()
                }}
              </p>
            </div>
            <div class="mt-4" v-if="item.transaction.childTransaction">
              <h6 class="details-header">Child Transaction</h6>
              <p class="details-body">
                {{
                  item.transaction.currency.symbol +
                  Number(
                    item.transaction.childTransaction.amount
                  ).toLocaleString()
                }}
              </p>
        ...
        <div v-if="item.status === 'pending'">
          ...
          <div
            ...
            v-if="!confirmBox"
          >
            <button class="primary-btn" @click="changeStatus('confirm')">
              Confirm
            </button>
            <button class="danger-btn" @click="changeStatus('decline')">
              Decline
            </button>
          </div>
          <div v-if="confirmBox">
            <h6 class="details-header">Upload Payment Proof</h6>
            <input type="file" id="file-ip-1" @change="addProof($event)" />
            <div class="d-flex align-items-center mt-3" style="gap: 10px">
              <button class="primary-btn" @click="changeStatus('confirm')">
                Submit
              </button>
              <button class="outline-btn" @click="confirmBox = !confirmBox">
                Cancel
              </button>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...mapActions("deposits", ["updateStatus", "view"]),
    changeStatus(value) {
